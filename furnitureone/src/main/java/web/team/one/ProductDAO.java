...
public class ProductDAO {
	...
	//내가 판매중인 상품 전체 가져오기
		...
		//상품명을 검색했을때의 해당 개수 가져오기
		   public int getProductSearchCount(String search, String id) {
		      ...
		      try {
		         conn = getConnection();
		         String sql="select count(*) from product where pcon=0 and mid=? and pname like '%"+search+"%'";
		         pstmt=conn.prepareStatement(sql);
		         pstmt.setString(1, id);
		         rs = pstmt.executeQuery();
		   ...
		   public int getProductSearchCount(String search) {
			   ...
			   try {
				   conn = getConnection();
				   String sql="select count(*) from product where pcon=0 and pname like '%"+search+"%'";
				   pstmt=conn.prepareStatement(sql);
				   rs = pstmt.executeQuery();
		   ...
		   public List<ProductDTO> getProductSearch(int start,int end,String search) {
			   ...
			   try {
				   ...
				   String sql="select B.* from (select rownum r, A.* from "
						   +"(select * from product where pcon=0 and pname like '%"+search+"%')A)B"
						   + " where r >= ? and r <= ?";
				   pstmt=conn.prepareStatement(sql);
	...
	//모든 상품 세어주는 메서드(성민)
		public int getAllProductCount() {
			...
			try {
				conn = getConnection();
				String sql="select count(*) from product where pcon=0";
				pstmt=conn.prepareStatement(sql);
				rs = pstmt.executeQuery();
	...
	// 페이징 처리해서 모든 리스트 가져와주는 메서드(성민)
		public List getAllProductList(int start, int end) {
			...
			try {
				conn = getConnection();
				String sql="select B.* from (select rownum r, A.* from (select * from product where pcon=0 order by preg desc) A) B where r >= ? and r <= ?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, start);
				pstmt.setInt(2, end);
				rs = pstmt.executeQuery();
	...
	//색에 따른 물품추천해서 가져오기(성민)
			...
			try {
				...
				String sql= " ";
					if(color.equals("white")) {
						sql="select B.* from (select A.* from (select * from product where pcon=0 order by preg desc) A order by pgrade desc)B order by psellst desc";
						
					}else if(color.equals("gray")) {
						sql="select B.* from (select A.* from (select * from product where pcon=0 and  pcolor='gray' or pcolor='black' order by preg desc) A order by pgrade desc)B order by psellst desc";
						
					}else if(color.equals("black")) {
						sql="select B.* from (select A.* from (select * from product where pcon=0 and  pcolor='gray' or pcolor='black' order by preg desc) A order by pgrade desc)B order by psellst desc";
						
					}else if(color.equals("brown")) {
						sql="select B.* from (select A.* from (select * from product where pcon=0 and   pcolor='brown' or pcolor='red' order by preg desc) A order by pgrade desc)B order by psellst desc";
						
					}else if(color.equals("navy")) {
						sql="select B.* from (select A.* from (select * from product where pcon=0 and   pcolor='gray' or pcolor='navy' order by preg desc) A order by pgrade desc)B order by psellst desc";
						
					}else if(color.equals("red")) {
						sql="select B.* from (select A.* from (select * from product where pcon=0 and   pcolor='brown' or pcolor='red' order by preg desc) A order by pgrade desc)B order by psellst desc";
						
					}else if(color.equals("green")) {
						sql="select B.* from (select A.* from (select * from product where pcon=0 and   pcolor='black' or pcolor='green' order by preg desc) A order by pgrade desc)B order by psellst desc";
					}	
	    ...
	    //상세 검색 물품 물품 타입& 색상 & 가격7
	      public List<ProductDTO> getSearchProductList(String productType, int fPrice, int ePrice,String color1) {
	    	  Connection conn = null;
	    	  PreparedStatement pstmt = null;
	    	  ResultSet rs = null;
	    	  List<ProductDTO> list = null;
	    	  try {
	    		  conn = getConnection();
	    		  String sql="select * from product where pcon=0 and ptype='"+productType+"' and pcolor='"+color1+"' and pprice>"+fPrice+" and pprice<"+ePrice+" order by preg desc";
	    		  pstmt = conn.prepareStatement(sql);
	    		  rs = pstmt.executeQuery();
	    		  if(rs.next()) {
	    			  list = new ArrayList<ProductDTO>();
	      ...
	      //상세 검색 물품 물품 색상 & 가격7 페이징
	      public List<ProductDTO> getSearchProductList( String productType,int fPrice, int ePrice,String color1, int start,int end) {
	    	  Connection conn = null;
	    	  PreparedStatement pstmt = null;
	    	  ResultSet rs = null;
	    	  List<ProductDTO> list = null;
	    	  try {
	    		  conn = getConnection();
	    		  String sql="select B.* from (select rownum r, A.* from "
	    				  +"(select * from product where pcon=0 and ptype='"+productType+"' and pcolor='"+color1+"' and pprice>"+fPrice+" and pprice<"+ePrice+" order by preg desc)A)B"
	    				  + " where r >= ? and r <= ?";
	    		  pstmt = conn.prepareStatement(sql);
	    		  pstmt.setInt(1, start);
	    		  pstmt.setInt(2, end);
