...
public class MemberDAO {
	...
	//id 찾기
	public int idfindCount(String mname, String memail) {
		//String result = null;
		int count = 0;
		Connection conn = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			String sql = "select count(*) from member where mname=? and memail=?";
			pstmt = conn.prepareStatement(sql);
	...
	//pw 찾기
	public int pwfindCount(String mid, String memail) {
		//String result = null;
		int count = 0;
		Connection conn = null;
		PreparedStatement pstmt = null;
		...
		try {
			conn = getConnection();
			//사용자가 작성한 mname와 동일한 memail값이 DB에 있는 꺼내오기
			String sql = "select count(*) from member where mid=? and memail=?";
			pstmt = conn.prepareStatement(sql);
			pstmt.setString(1, mid);
			pstmt.setString(2, memail);
			rs = pstmt.executeQuery();
	...
	//계정 정지
	public void memberLongStop(int mnum) {
		Connection conn = null;
		PreparedStatement pstmt = null;
			
		try {
			conn = getConnection();
			String sql="update member set mcon=2 where mnum=?";
			pstmt=conn.prepareStatement(sql);
			pstmt.setInt(1, mnum);
			pstmt.executeUpdate();
	...
	//계정 복구 8.1
	public void memberBack(int mnum) {
		Connection conn = null;
		PreparedStatement pstmt = null;
			
		try {
			conn = getConnection();
			String sql="update member set mcon=0 where mnum=?";
			pstmt=conn.prepareStatement(sql);
			pstmt.setInt(1, mnum);
			pstmt.executeUpdate();
	...
	//해당 페이지에 띄워줄 글 가져오기
		...
		public List getSellers() {
			...
			try {
				conn = getConnection(); 
				String sql ="select mnum from sellregis where scon=1 order by sreg desc";
				pstmt = conn.prepareStatement(sql);
				rs = pstmt.executeQuery();
		...
		public int getSelMemCount() {
			...
			try {
				conn = getConnection(); 
				String sql ="select count(*) from member where mtype='seller'";
				pstmt = conn.prepareStatement(sql);
				rs = pstmt.executeQuery();
				if(rs.next()) {
					count = rs.getInt(1);
		...
		public int getSelMemSearchCount(String search) {
			...
			try {
				conn = getConnection(); 
				String sql ="select count(*) from member where mid=? and mtype='seller'";
				pstmt = conn.prepareStatement(sql);
		...
		public List getSelMemList(int start,int end) {
			...
			try {
				conn = getConnection(); 
				String sql = "select B.* from (select rownum r, A.* from (select * from member where mtype='seller' order by mreg desc) A) B where r >= ? and r <= ?"; 
				pstmt = conn.prepareStatement(sql);
		...
		public List getSelMemSearch(int start,int end,String search) {
			...
			try {
				conn = getConnection(); 
				String sql ="select B.* from (select rownum r, A.* from (select * from member where mid=? and mtype='seller' order by mreg desc) A) B where r >= ? and r <= ?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, search);
				pstmt.setInt(2, start);
				pstmt.setInt(3, end);
		...
		public int getMcon(String mid) {
			...
			try {
				conn = getConnection();
				String sql = "select mcon from member where mid=?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, mid);
				rs = pstmt.executeQuery();
				if(rs.next()) {
