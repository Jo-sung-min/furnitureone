...
<%
   ...
   // DB에서 전체 글 개수 가져오기 
   int count = 0; 
   List buyMemList = null;  // 글 가져올 변수 미리 선언 
   ...
   if(search != null) { // 검색일때 
	   if(memDAO.confirmId(search)){
	      count = memDAO.getBuyMemSearchCount(search);  // 검색에 맞는 게시글에 개수 가져오기 
	      number = count - (currentPage - 1) * pageSize;
	      if(count > 0) { 
	         // 검색한 글 목록 가져오기 
	         buyMemList = memDAO.getBuyMemSearch(startRow, endRow, search);
	      } 
	   }else{%>
		 <script>
		 	alert('존재하지않는 ID입니다');
      ...
      count = memDAO.getBuyMemCount();  // 그냥 전체 게시글 개수 가져오기
      // 글이 하나라도 있으면, 현재 페이지에 띄워줄 만큼만 가져오기 (페이지 번호 고려) 
      if(count > 0){
    	  buyMemList = memDAO.getBuyMemList(startRow, endRow); 
      }
   }
%>
<body>
    	 ...
    	 <tr>
		   ...
		   <% if(count > 0) { 
		      ...
		      if(startPage > pageNumSize) { 
		         if(search != null) { %>
		            <a class="pageNums" href="buyMemListForm.jsp?pageNum=<%=startPage-1%>&search=<%=search%>"> &lt; &nbsp; </a>
		         <%}else{%>
		            <a class="pageNums" href="buyMemListForm.jsp?pageNum=<%=startPage-1%>"> &lt; &nbsp; </a>
		         <%}
		      }
		      
		      for(int i = startPage; i <= endPage; i++) { 
		         if(search != null) { %>
		            <a class="pageNums" href="buyMemListForm.jsp?pageNum=<%=i%>&search=<%=search%>"> &nbsp; <%= i %> &nbsp; </a>
		         <%}else{ %>
		            <a class="pageNums" href="buyMemListForm.jsp?pageNum=<%=i%>"> &nbsp; <%= i %> &nbsp; </a> 
		         <%} 
		      }
		      
		      if(endPage < pageCount) { 
		         if(search != null) { %>
		            <a class="pageNums" href="buyMemListForm.jsp?pageNum=<%=startPage+pageNumSize%>&search=<%=search%>"> &nbsp; &gt; </a>
		      <%   }else{ %>
		            <a class="pageNums" href="buyMemListForm.jsp?pageNum=<%=startPage+pageNumSize%>"> &nbsp; &gt; </a>
		      <%   }
		      } 
		      ...
		      <%-- 작성자/내용 검색 --%>
		      <form action="buyMemListForm.jsp">
		         <input type="text" name="search" placeholder="ID를 검색하세요" /> 
		         <input type="submit" value="검색" />
		      </form>
		      <br />
		      
		      <button onclick="window.location='buyMemListForm.jsp'"> 전체 목록 </button>
		   </div1>
   		</td>
	   </tr>
   </table>
