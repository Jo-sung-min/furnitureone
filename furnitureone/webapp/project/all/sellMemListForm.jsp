...
<%
   ...
   if(search != null) { // 검색일때 
	   if(memDAO.confirmId(search)){
	      count = memDAO.getSelMemSearchCount(search);  // 검색에 맞는 게시글에 개수 가져오기 
	      number = count - (currentPage - 1) * pageSize;
	      if(count > 0) { 
	         // 검색한 글 목록 가져오기 
	         sellMemList = memDAO.getSelMemSearch(startRow, endRow, search); 
	      }
	   }else{%>
		 <script>
		 	alert('존재하지 않는 ID입니다');
      ...
      count = memDAO.getSelMemCount();  // 그냥 전체 게시글 개수 가져오기
      // 글이 하나라도 있으면, 현재 페이지에 띄워줄 만큼만 가져오기 (페이지 번호 고려) 
      if(count > 0){
    	  sellMemList = memDAO.getSelMemList(startRow, endRow); 
      }
   }
%>
<body> 
	...
	<div class="side1">
  	...
  	<br/><br/><br/>	
   	<table>	
   		<tr>
   			<td>
				<form action="memberListForm.jsp" method="post">
					<table width="680"height="500">
						<tr height="60">
							<td colspan="6"><h2>판매회원목록</h2></td>
						</tr>
						<tr height="30">
							<td>아이디</td>
							<td>이름</td>
						...
						<%if(sellMemList != null){ 
							for(int i = 0; i< sellMemList.size();i++){
								...
								<tr>
									...
									<td><input type="button" value="회원 상세정보" onclick="window.location.href='memberListForm.jsp?pageNum=<%=pageNum%>&mnum=<%=memDTO.getMnum()%>'"/></td>
								</tr>
							<%}%>
						<%}else{ %>
							<tr>
								<td colspan="6"><h3>회원이 없습니다.</h3></td>
							</tr>
						<%} %>
						<tr height="30">
							<td align="right" colspan="6"><input type="button" value="뒤로" onclick="window.location.href='mypageAdminForm.jsp'"/></td>
						</tr>
					</table>
				</form>
			</td>
   		...
   		<tr>
			   ...
			   <% if(count > 0) { 
			      ...
			      if(startPage > pageNumSize) { 
			         if(search != null) { %>
			            <a class="pageNums" href="sellMemListForm.jsp?pageNum=<%=startPage-1%>&search=<%=search%>"> &lt; &nbsp; </a>
			         <%}else{%>
			            <a class="pageNums" href="sellMemListForm.jsp?pageNum=<%=startPage-1%>"> &lt; &nbsp; </a>
			         <%}
			      }
			      
			      for(int i = startPage; i <= endPage; i++) { 
			         if(search != null) { %>
			            <a class="pageNums" href="sellMemListForm.jsp?pageNum=<%=i%>&search=<%=search%>"> &nbsp; <%= i %> &nbsp; </a>
			         <%}else{ %>
			            <a class="pageNums" href="sellMemListForm.jsp?pageNum=<%=i%>"> &nbsp; <%= i %> &nbsp; </a> 
			         <%} 
			      }
			      
			      if(endPage < pageCount) { 
			         if(search != null) { %>
			            <a class="pageNums" href="sellMemListForm.jsp?pageNum=<%=startPage+pageNumSize%>&search=<%=search%>"> &nbsp; &gt; </a>
			      <%   }else{ %>
			            <a class="pageNums" href="sellMemListForm.jsp?pageNum=<%=startPage+pageNumSize%>"> &nbsp; &gt; </a>
			      <%   }
			      } 
			      ...
			      <%-- 작성자/내용 검색 --%>
			      <form action="sellMemListForm.jsp">
			         <input type="text" name="search" placeholder="ID를 검색하세요" /> 
			         <input type="submit" value="검색" />
			      </form>
			      <br />
			      <button onclick="window.location='sellMemListForm.jsp'"> 전체 목록 </button>
			   </div>
		   	</td>
		</tr>
   </table>
